...
{
  if (m_mesh == 0) {
    printf("starting building tri mesh");
    m_mesh = tCQMMesh2Builder().buildTriMesh(m_fname);
    printf("starting get num verts\n");
    m_triverts = m_mesh->getNumVerts();
    printf("start get num edges\n");
    m_triedges = m_mesh->getNumEdges();
    printf("start get num faces\n");
    m_trieles = m_mesh->getNumFaces();
    printf("end compute triangulation\n");
  } else {
    printf("m_mesh != 0\n");
  }
}
...
tCQMGeneralControl::computeConvexQuadrangulation()
{
  ...
  if (m_lcquads == 0) {
    ...
    m_lcquads = compquad.computeConvexQuadrangulation(m_fname+"_spt.txt");
    //std::cout << "filename: " << m_fname+"_spt.txt" << std::endl;
